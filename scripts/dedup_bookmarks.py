...
def main() -> None:
    """Remove duplicate bookmarks by (user_id, film_id), keeping the newest by created_at."""
    db = MongoClient(settings.mongo_dsn)[settings.mongo_db]
    col = db["bookmarks"]

    # Find (user_id, film_id) keys having more than one document.
    pipeline = [
        {
            "$group": {
                "_id": {"user_id": "$user_id", "film_id": "$film_id"},
                "count": {"$sum": 1},
            }
        },
        {"$match": {"count": {"$gt": 1}}},
    ...
    # For each key, keep the newest doc (by created_at) and delete the rest.
    for d in dups:
        key = d["_id"]
