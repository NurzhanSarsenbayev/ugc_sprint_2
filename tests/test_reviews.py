...
async def test_review_create_and_get_by_id(client):
    ...
    r = await client.post(BASE, json={"film_id": film, "text": "hello"}, headers=uid_header(author))
...
async def test_review_update_forbidden_for_stranger_returns_404(client):
    ...
    rid = (
        await client.post(BASE, json={"film_id": film, "text": "t"}, headers=uid_header(author))
    ).json()["review_id"]
    r = await client.patch(f"{BASE}/{rid}", json={"text": "x"}, headers=uid_header(stranger))
...
async def test_reviews_list_sorted_new_contains_recent_items(client):
    film, author = new_film(), new_user()
    r1 = await client.post(BASE, json={"film_id": film, "text": "a"}, headers=uid_header(author))
    r2 = await client.post(BASE, json={"film_id": film, "text": "b"}, headers=uid_header(author))
...
async def test_reviews_list_sorted_top_orders_by_votes_up_then_created_desc(client):
    film, u1, u2 = new_film(), new_user(), new_user()
    r1 = await client.post(BASE, json={"film_id": film, "text": "a"}, headers=uid_header(u1))
    r2 = await client.post(BASE, json={"film_id": film, "text": "b"}, headers=uid_header(u1))
    rid1, rid2 = r1.json()["review_id"], r2.json()["review_id"]
    # boost the second review (votes.up = 1)
    await client.post(f"{BASE}/{rid2}/vote", json={"value": "up"}, headers=uid_header(u2))
...
async def test_review_votes_up_down_unvote_updates_stats(client):
    ...
    rid = (
        await client.post(BASE, json={"film_id": film, "text": "ok"}, headers=uid_header(u))
    ).json()["review_id"]

    await client.post(f"{BASE}/{rid}/vote", json={"value": "up"}, headers=uid_header(u))
    s = await read_stats(client, film)
    assert s["votes_up"] == 1 and s["votes_down"] == 0

    await client.post(f"{BASE}/{rid}/vote", json={"value": "down"}, headers=uid_header(u))
    s = await read_stats(client, film)
